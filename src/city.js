...
function chooseColonizerRoom(myCities){
    if(!Memory.flags.claim && !Game.flags.unclaim){
        return 0;
    }
    let goodCities = _.filter(myCities, city => city.controller.level >= 4 && Game.spawns[city.memory.city]);
    let claimRoom = Game.flags.unclaim ? Game.flags.unclaim.pos.roomName: Memory.flags.claim.pos.roomName
...
function updateColonizers(city, memory, closestRoom) {
    ...
    if(roomName == closestRoom){
        ...
        if(Memory.flags.claim && Memory.flags.claim.room && Memory.flags.claim.room.controller.my){
            memory[rC.name] = 0;
        } else {
            memory[rC.name] = Memory.flags.claim ? 1 : 0;
        }
        if(Game.time % 1000 === 0){
