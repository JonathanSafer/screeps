...
export function loop() {
    ...
    profiler.wrap(function () {
        ...
        if(Game.rooms["E25S15"]) {
            ...
            if(needHaulers) {
                const haulersNeeded = Math.ceil((reactor.store.getFreeCapacity(RESOURCE_THORIUM)) / 200)
        ...
        if(Game.rooms["E35S15"]) {
            ...
            let needClaimer = false
            let needHaulers = false
            if(reactor.owner.username != "Yoner" && !(Memory.reactorEndTime > Game.time)){
                needClaimer = true
            }
            needHaulers = true
            if(needHaulers) {
        ...
        if(Game.rooms["E28S12"]) {
            const hostiles = u.findHostileCreeps(Game.rooms["E28S12"])
            const ramparts = _.filter(Game.rooms["E28S12"].find(FIND_MY_STRUCTURES), struct => struct.structureType == STRUCTURE_RAMPART) as StructureRampart[]
            if(hostiles.length > 0 && ramparts[0].isPublic) {
                for(const rampart of ramparts) {
                    rampart.setPublic(false)
                }
            } else if (hostiles.length == 0 && !ramparts[0].isPublic) {
                for(const rampart of ramparts) {
